        ...
        <td className="align-middle" colSpan="2">
          ...
          <InputGroup>
            <div className="input-group-prepend">
              <InputGroup.Text>
                <Form.Check className='form-check' type="checkbox" aria-label="Checkbox for maximum value"
                            checked={this.props.element.lmax & CONSTRAINED}
                            onChange={this.props.element.lmax & CONSTRAINED
                              ? this.onResetFlagMaxConstrained
                              : this.onSetFlagMaxConstrained}
                            disabled={this.props.element.lmax & FIXED ? true : false}/>
              </InputGroup.Text>
            </div>
            <FormControlTypeNumber id={this.props.element.name + '_cmax'} icon_alerts={icon_alerts}
                                   className={className} value={this.props.element.cmax}
                                   validmin={this.props.element.validmin} validmax={this.props.element.validmax}
                                   disabled={this.props.element.lmax & FIXED || this.props.element.lmax & CONSTRAINED
                                     ? false
                                     : true} disabledText={this.props.element.lmax & CONSTRAINED ? false : true}
                                   onChangeValid={this.onChangeValidMaxConstraint}
                                   onChangeInvalid={this.onChangeInvalidMaxConstraint} onClick={this.onClick}/>
          </InputGroup>
                ...
                <Table borderless="true">
                  ...
                  <tr>
                    ...
                    <td>
                      <InputGroup>
                        <ButtonGroup>
                          {this.props.element.cmaxchoices.map((e) => {
                            return (
                              <Button key={e} variant="primary" onClick={(event) => {this.onVariableButton(event, e)}}
                                      style={{ marginBotton: '5px' }}
                                      active={this.props.element.cmaxchoices[this.props.element.cmaxchoice] ===
                                      e}>{e}</Button>
                            )
                          })}
                        </ButtonGroup>
                  ...
                  <tr>
                    ...
                    <td>
                      <InputGroup>
                        <FormControlTypeNumber id={this.props.element.name + '_cmax'} className={className}
                                               value={this.props.element.cmax} validmin={this.props.element.validmin}
